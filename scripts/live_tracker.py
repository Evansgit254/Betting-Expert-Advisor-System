...
class LiveOddsTracker:
    ...
    def check_opportunities(self):
        ...
        try:
            ...
            odds = self.fetcher.get_odds(
                list(fixtures['market_id']) if not fixtures.empty else []
            )
            ...
            if self._check_count > 0:
                send_alert(
                    f"⚠️ Error in live tracker (check #{self._check_count}): {e}",
                    level='warning'
                )
    ...
    def alert_opportunities(self, opportunities):
        ...
        for i, bet in enumerate(opportunities, 1):
            ...
            print(f"   Odds: {bet['odds']:.2f} | Prob: {bet['p']:.1%} | "
