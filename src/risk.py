...
from src.db import get_session, get_consecutive_losses, get_peak_bankroll, get_current_bankroll
...
def kelly_stake(p: float, odds: float, bank: float, kelly_frac: float = 0.25) -> float:
    ...
    try:
        ...
        logger.debug(f"Kelly stake: p={p:.3f}, odds={odds:.2f}, bank={bank:.2f}, "
...
def stake_from_bankroll(
    ...
    multiplier = kelly_multiplier if kelly_multiplier is not None else settings.DEFAULT_KELLY_FRACTION
...
def check_risk_limits(
    ...
    if daily_loss >= settings.DAILY_LOSS_LIMIT:
        ...
        send_alert(
            f"⚠️ Daily loss limit reached: ${daily_loss:.2f}",
            level='warning'
        )
        return {
            "allowed": False,
    ...
    if daily_loss + potential_loss > settings.DAILY_LOSS_LIMIT:
        ...
        return {
            "allowed": False,
            "reason": "Bet would risk exceeding daily loss limit"
        }
    ...
    if open_bets_count >= settings.MAX_OPEN_BETS:
        ...
        return {
            "allowed": False,
            "reason": f"Maximum open bets ({settings.MAX_OPEN_BETS}) reached"
        }
    ...
    if not is_dry_run:
        with get_session() as session:
            ...
            if consecutive_losses >= max_consecutive:
                logger.error(
                    f"Circuit breaker triggered: {consecutive_losses} consecutive losses"
                )
                send_alert(
...
def calculate_variance(probability: float, odds: float, stake: float = 1.0) -> float:
    ...
    try:
        ...
        mean = Decimal(str(calculate_expected_value(probability, float(odds_dec), float(stake_dec))))
...
def get_recommended_stake(
    ...
    try:
        p_dec = Decimal(str(probability))
        odds_dec = Decimal(str(odds))
        edge = float((p_dec * odds_dec - Decimal('1')).quantize(
