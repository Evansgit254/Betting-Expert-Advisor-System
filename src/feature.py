...
def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.debug(f"Temporal features: days_until_match range [{df['days_until_match'].min():.1f}, {df['days_until_match'].max():.1f}]")
...
def select_features(df: pd.DataFrame, feature_names: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    if not available:
        ...
        exclude_cols = [
            'market_id', 'start', 'sport', 'league', 'selection', 'provider',
            'home', 'away',
            'home_strength', 'away_strength', 'strength_differential',
            'home_favorite', 'odds_differential', 'odds_ratio'
        ]
