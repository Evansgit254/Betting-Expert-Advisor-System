...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, JSON, Boolean, event, func, text, case
...
def update_bet_result(bet_id: int, result: str, profit_loss: float) -> bool:
    ...
    logger.debug("update_bet_result called with bet_id=%s, result=%s, profit_loss=%.2f", 
    ...
    with handle_db_errors() as session:
        ...
        logger.debug("Bet found: id=%s, result=%s, settled_at=%s", 
                     bet.id, bet.result, bet.settled_at)
...
def get_daily_loss(date: Optional[Union[datetime, date]] = None) -> float:
    ...
    with handle_db_errors() as session:
        ...
        with session.begin():
            ...
            logger.debug(
                "Calculated daily loss for %s: %.2f", 
                start_of_day.date(), float(total_loss)
            )
...
def get_consecutive_losses(session: Session, max_recent: int = 10) -> int:
    ...
    recent_bets = session.query(BetRecord).filter(
        BetRecord.is_dry_run == False,
        BetRecord.result.in_(['win', 'loss'])
    ).order_by(BetRecord.settled_at.desc()).limit(max_recent).all()
...
def get_peak_bankroll(session: Session, days: int = 30) -> float:
    ...
    peak = session.query(func.max(DailyStats.ending_bankroll)).filter(
